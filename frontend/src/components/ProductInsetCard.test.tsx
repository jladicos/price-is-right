...
describe('ProductInsetCard', () => {
  ...
  describe('Price Formatting Edge Cases (Critical)', () => {
    ...
    it('should handle very large prices', () => {
      ...
      expect(screen.getByTestId('product-inset-price')).toHaveTextContent('$999,999.99');
    });
  ...
  describe('Showcase Mode', () => {
    ...
    it('should render first product in showcase mode', () => {
      render(<ProductInsetCard products={mockShowcaseProducts} isVisible={true} />);

      expect(screen.getByTestId('product-inset-card')).toBeInTheDocument();
      expect(screen.getByTestId('product-inset-name')).toHaveTextContent('Laptop Computer');
    });
    ...
    it('should NOT show carousel indicator with single product', () => {
      ...
      expect(screen.queryByTestId('carousel-indicator')).not.toBeInTheDocument();
    });
    ...
    it('should navigate to next product when next button clicked', () => {
      ...
      expect(screen.getByTestId('product-inset-name')).toHaveTextContent('Wireless Headphones');
      expect(screen.getByTestId('carousel-indicator')).toHaveTextContent('2 / 3');
    });
    ...
    it('should wrap to first product when clicking next on last product', () => {
      ...
      expect(screen.getByTestId('product-inset-name')).toHaveTextContent('Smart Watch');
  ...
  describe('Position Prop', () => {
    it('should position card on right by default', () => {
      render(<ProductInsetCard product={mockProduct} productId="test" isVisible={true} />);

      const card = screen.getByTestId('product-inset-card');
      // Check that right style is applied (not left)
      // Component uses responsive value { base: "10px", lg: "20px" }, test env uses base
      expect(card).toHaveStyle({ right: '10px' });
    });
    ...
    it('should position card on left when position="left"', () => {
      ...
      const card = screen.getByTestId('product-inset-card');
      // Component uses responsive value { base: "10px", lg: "20px" }, test env uses base
      expect(card).toHaveStyle({ left: '10px' });
    });
  ...
  describe('Showcase Total Value Display', () => {
    ...
    it('should show "TOTAL VALUE" label in showcase mode when price is shown', () => {
      ...
      expect(screen.getByTestId('showcase-total-label')).toHaveTextContent('TOTAL VALUE');
    });
    ...
    it('should NOT show "TOTAL VALUE" label in single product mode', () => {
      ...
      expect(screen.queryByTestId('showcase-total-label')).not.toBeInTheDocument();
    });

    it('should NOT show total value label when price is not shown', () => {
      render(
        <ProductInsetCard products={mockShowcaseProducts} isVisible={true} showPrice={false} />,
      );
    ...
    it('should format total showcase value correctly', () => {
      ...
      expect(screen.getByTestId('product-inset-price')).toHaveTextContent('$1,599.98');
  ...
  describe('Backward Compatibility (Single Product Mode)', () => {
    ...
    it('should prioritize products prop over product prop when both provided', () => {
      ...
      expect(screen.getByTestId('product-inset-name')).toHaveTextContent('Showcase Item');
    });
  });
});
