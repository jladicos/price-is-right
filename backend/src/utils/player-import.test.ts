...
describe('Player Import Logic', () => {
  ...
  describe('Weight database integration', () => {
    ...
    it('should handle boundary values (0 and 1)', () => {
      ...
      const min = db.prepare('SELECT * FROM players WHERE access_code = ?').get(code1) as any;
      const max = db.prepare('SELECT * FROM players WHERE access_code = ?').get(code2) as any;
