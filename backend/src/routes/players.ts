...
const playersRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  }>('/players', { preHandler: [authenticateRequest, requireHost] }, async (request) => {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (photoFile) {
        ...
        const extension = getFileExtension(photoFile.filename, photoFile.mimetype);
        photoFilename = generatePlayerPhotoFilename(firstName, lastName, extension);

        // Save file
        const photoDir = path.join(__dirname, '..', '..', 'public', 'images', 'players');
  ...
  fastify.put<{
  ...
  }>('/players/:id', { preHandler: [authenticateRequest, requireHost] }, async (request, reply) => {
    const db = getDatabase();
    ...
    if (weight !== undefined) {
      if (typeof weight !== 'number' || isNaN(weight)) {
        return reply.status(400).send({ error: 'Weight must be a valid number' });
      }
      // Clamp to [0, 1] range
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const filename = generatePlayerPhotoFilename(player.firstName, player.lastName, extension);
