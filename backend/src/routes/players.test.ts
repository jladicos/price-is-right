...
describe('Players API Routes', () => {
  ...
  describe('PUT /api/players/:id', () => {
    ...
    describe('Weight validation', () => {
      it('should update player weight to valid value (0.5)', async () => {
        const result = db
          .prepare(
            'INSERT INTO players (first_name, last_name, access_code, role) VALUES (?, ?, ?, ?)',
          )
        ...
        expect(response.statusCode).toBe(200);
        const body = JSON.parse(response.body);
        expect(body.player.weight).toBe(0.5);
      });

      it('should update player weight to 0.0 (backup tier)', async () => {
        const result = db
          .prepare(
            'INSERT INTO players (first_name, last_name, access_code, role) VALUES (?, ?, ?, ?)',
          )
        ...
        expect(response.statusCode).toBe(200);
        const body = JSON.parse(response.body);
        expect(body.player.weight).toBe(0.0);
      });

      it('should update player weight to 1.0 (full probability)', async () => {
        const result = db
          .prepare(
            'INSERT INTO players (first_name, last_name, access_code, role) VALUES (?, ?, ?, ?)',
          )
        ...
        expect(response.statusCode).toBe(200);
        const body = JSON.parse(response.body);
        expect(body.player.weight).toBe(1.0);
      });

      it('should clamp weight > 1.0 to 1.0', async () => {
        const result = db
          .prepare(
            'INSERT INTO players (first_name, last_name, access_code, role) VALUES (?, ?, ?, ?)',
          )
        ...
        expect(response.statusCode).toBe(200);
        const body = JSON.parse(response.body);
        expect(body.player.weight).toBe(1.0); // Clamped to 1.0
      });

      it('should clamp weight < 0.0 to 0.0', async () => {
        const result = db
          .prepare(
            'INSERT INTO players (first_name, last_name, access_code, role) VALUES (?, ?, ?, ?)',
          )
        ...
        expect(response.statusCode).toBe(200);
        const body = JSON.parse(response.body);
        expect(body.player.weight).toBe(0.0); // Clamped to 0.0
      });
      ...
      it('should reject NaN weight', async () => {
        ...
        expect(response.statusCode).toBe(400);
        const body = JSON.parse(response.body);
        expect(body.error).toContain('Weight must be a valid number');
      });

      it('should update weight along with other fields', async () => {
        const result = db
          .prepare(
            'INSERT INTO players (first_name, last_name, access_code, role) VALUES (?, ?, ?, ?)',
          )
        ...
        const body = JSON.parse(response.body);
        expect(body.player.firstName).toBe('Johnny');
        expect(body.player.weight).toBe(0.75);
      });

      it('should not modify weight if not provided', async () => {
        const result = db
          .prepare(
            'INSERT INTO players (first_name, last_name, access_code, role, weight) VALUES (?, ?, ?, ?, ?)',
          )
          .run('John', 'Doe', 'JOHN01', 'player', 0.7);
        ...
        const body = JSON.parse(response.body);
        expect(body.player.firstName).toBe('Johnny');
        expect(body.player.weight).toBe(0.7); // Unchanged
      });
    });
  });
