...
import { exportDatabase, importDatabase, type DatabaseExport } from './export-import.js';
...
describe('Export/Import Database Functions', () => {
  ...
  describe('exportDatabase', () => {
    ...
    it('should export players with correct fields', () => {
      ...
      db.prepare(
        `INSERT INTO players (first_name, last_name, email, access_code, photo_filename, role, active)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
      ).run('John', 'Doe', 'john@example.com', 'JOHN01', 'john.jpg', 'player', 1);
    ...
    it('should NOT export session tokens (security)', () => {
      ...
      db.prepare(
        `INSERT INTO players (first_name, last_name, access_code, photo_filename, role, active, session_token)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
      ).run('Jane', 'Smith', 'JANE01', 'default.jpg', 'host', 1, 'secret-session-token-123');
    ...
    it('should export active and inactive players', () => {
      ...
      expect(exported.players).toHaveLength(2);
      expect(exported.players.find((p) => p.firstName === 'Active')?.active).toBe(1);
      expect(exported.players.find((p) => p.firstName === 'Inactive')?.active).toBe(0);
    });
  });
  ...
  describe('importDatabase', () => {
    it('should reject invalid data structure', () => {
      ...
      expect(() => importDatabase(db, invalidData)).toThrow('Invalid export file format');
    });
    ...
    it('should reject unsupported version', () => {
      ...
      expect(() => importDatabase(db, futureVersion)).toThrow('Unsupported export version: 2.0');
    });
    ...
    it('should import empty database successfully', () => {
      const emptyData: DatabaseExport = {
        ...
        exportedAt: new Date().toISOString(),
        players: [],
        gameState: [{ key: 'game_enabled', value: 'true', updatedAt: new Date().toISOString() }],
      };
    ...
    it('should import game state correctly', () => {
      const data: DatabaseExport = {
        ...
        players: [],
        gameState: [
          { key: 'game_enabled', value: 'false', updatedAt: '2025-01-01 00:00:00' },
          { key: 'custom_setting', value: 'test_value', updatedAt: '2025-01-01 00:00:00' },
        ],
      };

      importDatabase(db, data);

      const gameState = db.prepare('SELECT * FROM game_state ORDER BY key').all();
    ...
    it('should import multiple players correctly', () => {
      const data: DatabaseExport = {
        ...
        gameState: [{ key: 'game_enabled', value: 'false', updatedAt: new Date().toISOString() }],
      };
  ...
  describe('Round-Trip (Export → Import)', () => {
    it('should preserve data through export/import cycle', () => {
      ...
      const originalPlayers = db.prepare('SELECT * FROM players ORDER BY id').all();
      const importedPlayers = db2.prepare('SELECT * FROM players ORDER BY id').all();
