...
describe('Contestants Database Functions', () => {
  ...
  describe('addContestantToRow', () => {
    it('should add a contestant to the row', () => {
      const contestant = addContestantToRow(1, 1, 'section_1', 'pending_reveal');
    ...
    it('should set timestamps', () => {
      ...
      expect(contestant.added_at).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/);
      expect(contestant.created_at).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/);
      expect(contestant.updated_at).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/);
    });
  ...
  describe('getAllActiveContestants', () => {
    it('should return active contestants from all segments', () => {
      ...
      expect(contestants.some((c) => c.game_segment === 'section_1')).toBe(true);
      expect(contestants.some((c) => c.game_segment === 'section_2')).toBe(true);
    });
  ...
  describe('revealContestant', () => {
    ...
    it('should update updated_at timestamp', () => {
      ...
      expect(revealed.updated_at).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/);
    });
  ...
  describe('replaceContestant', () => {
    ...
    it('should throw error if old contestant not found', () => {
      expect(() => replaceContestant(9999, 5, 'active')).toThrow('Contestant 9999 not found');
    });
  });
