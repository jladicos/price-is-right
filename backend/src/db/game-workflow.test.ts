...
describe("Game Workflow Database Functions", () => {
  ...
  describe("resetGame comprehensive validation", () => {
    ...
    it("should preserve players table across reset", () => {
      const db = getDatabase();
      ...
      expect(playersBefore.length).toBe(1); // Exactly 1 player from beforeEach
      ...
      expect(playersAfter[0]).toEqual(playersBefore[0]); // Verify full player object unchanged
    });
  });
