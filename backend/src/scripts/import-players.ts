...
export function parseWeight(value: string | number | undefined | null): number {
  // Default to 1.0 if empty or missing
...
function importPlayersFromXLSX(filePath: string): void {
  ...
  const existingCount = db.prepare('SELECT COUNT(*) as count FROM players').get() as {
    count: number;
  };
  ...
  let errorCount = 0;
  const errors: string[] = [];
  const hosts: Array<{ name: string; accessCode: string }> = [];

  const insertStmt = db.prepare(`
    INSERT INTO players (first_name, last_name, email, access_code, photo_filename, role, weight)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    try {
      ...
      if (!row['First Name'] || !row['Last Name'] || !row.Role) {
        throw new Error('Missing required fields (First Name, Last Name, or Role)');
      }
      ...
      // Parse and validate weight (defaults to 1.0, clamped to [0, 1])
      ...
      // Insert player
      insertStmt.run(firstName, lastName, email, accessCode, photoFilename, role, weight);

      // Track hosts for summary
      if (role === 'host') {
        hosts.push({ name: `${firstName} ${lastName}`, accessCode });
      }

      successCount++;
      console.log(`✓ Row ${rowNumber}: ${firstName} ${lastName} (${role}) - Code: ${accessCode}`);
    } catch (error) {
      errorCount++;
...
// This prevents process.exit() from being called when imported in tests
