...
// Insert test player
const existingPlayer = db.prepare('SELECT * FROM players WHERE access_code = ?').get('TEST01');
...
const existingHost = db.prepare('SELECT * FROM players WHERE access_code = ?').get('HOST01');
...
const existingAudience = db.prepare('SELECT * FROM players WHERE access_code = ?').get('AUD001');
...
if (!existingAudience) {
  ...
  console.log('✅ Test audience member already exists with access code: AUD001');
}
