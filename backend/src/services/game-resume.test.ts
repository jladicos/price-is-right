...
describe('Game Resume Service', () => {
  ...
  describe('resumeGame', () => {
    ...
    it('should verify complete workflow structure on resume', () => {
      ...
      expect(state.workflow.created_at).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/);
      expect(state.workflow.updated_at).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/);
    });
    ...
    it('should resume from showcase phase with metadata', () => {
      ...
      expect(state.workflow.phase_metadata).toBe('{"showcase_products": ["prod1", "prod2"]}');
    });
  });
});
